...
const OLMap: React.FC<OLMapProps> = ({
  ...
  }, [filteredGroups, pois, loadingPOIs, flockCameras, loadingFlockCameras]);
      ...
      initialMap.on("click", function (event) {
        ...
        initialMap.forEachFeatureAtPixel(
          ...
          function (feature, layer) {
            // Only check cluster layer (POIs are now in cluster layer)
            if (layer && layer.get("name") === "clusters") {
              ...
              if (clusterFeatures && clusterFeatures.length === 1) {
                ...
                // Multi-feature cluster - zoom to extent (works for all types)
        ...
        if (!featureFound) {
          ...
          // Force cluster layer to redraw to remove highlight
          const clusterLayer = initialMap
            .getLayers()
            .getArray()
            .find((layer) => layer.get("name") === "clusters");
  ...
  useEffect(() => {
    ...
    if (mapObjRef.current) {
      // Update cluster layer to reflect change
      const clusterLayer = mapObjRef.current
        .getLayers()
        .getArray()
        .find((layer) => layer.get("name") === "clusters");
  ...
  useEffect(() => {
    ...
    if (mapObjRef.current && currentPOI) {
      ...
      if (isMobile) {
        coordinates = fromLonLat([
          ...
          currentPOI.Latitude - 0.01, // Mobile offset
        ]);
      } else {
        coordinates = fromLonLat([
          currentPOI.Longitude - 0.005, // Desktop offset
          currentPOI.Latitude - 0.0025,
        ]);
      }
    ...
    if (mapObjRef.current) {
      // Redraw cluster layer to show POI highlight
      const clusterLayer = mapObjRef.current
        .getLayers()
        .getArray()
        .find((layer) => layer.get("name") === "clusters");
  ...
  useEffect(() => {
    ...
    if (mapObjRef.current && currentFlockCamera) {
      ...
      if (isMobile) {
        coordinates = fromLonLat([
          ...
          currentFlockCamera.Latitude - 0.01, // Mobile offset
        ]);
      } else {
        coordinates = fromLonLat([
          currentFlockCamera.Longitude - 0.005, // Desktop offset
          currentFlockCamera.Latitude - 0.0025,
        ]);
      }
    ...
    if (mapObjRef.current) {
      // Redraw cluster layer to show Flock Camera highlight
      const clusterLayer = mapObjRef.current
        .getLayers()
        .getArray()
        .find((layer) => layer.get("name") === "clusters");
