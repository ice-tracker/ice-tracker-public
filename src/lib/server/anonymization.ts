import { SENSITIVE_LOCATION_TYPES } from "@/constants/incident";

export const MIN_POPULATION = 500;
export const LEGACY_MISSING_LOCATION_RADIUS_MILES = 0.15;
...
export function isSensitiveLocation(location: string | null | undefined): boolean {
  ...
  return SENSITIVE_LOCATION_TYPES.has(normalized);
}
...
export function applyFixedRadiusOffset(
  longitude: number,
  latitude: number,
  radiusMiles: number,
  randomFn: () => number = Math.random
): { randomLat: number; randomLon: number; radius: number } {
  const milesPerDegreeLat = 69.0;
  const milesPerDegreeLon = 69.0 * Math.cos((latitude * Math.PI) / 180);
  const angle = randomFn() * 2 * Math.PI;
  const distance = radiusMiles * Math.sqrt(randomFn());
  const offsetLat = (distance * Math.sin(angle)) / milesPerDegreeLat;
  const offsetLon = (distance * Math.cos(angle)) / milesPerDegreeLon;
  return {
...
export async function processPoint(
  ...
  for (const radius of radii) {
    try {
      const { randomLat, randomLon } = applyFixedRadiusOffset(longitude, latitude, radius, randomFn);
  ...
  const { randomLat, randomLon } = applyFixedRadiusOffset(longitude, latitude, fallbackRadius, randomFn);
