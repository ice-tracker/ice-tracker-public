...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (type === "report") {
      ...
      // Luce bulk-upload rows carry a stable, globally-unique LogID. When
      // present, it's the authoritative idempotency key: re-uploading the
      // same sheet (or the same row twice) should always be treated as a
      // duplicate, even if coordinates or location text drifted slightly
      // between runs. Manual single/verified submissions have no LogID and
      // fall through to the coordinate-based check below.
      let existing;

      if (LogID) {
        existing = useTestTable
          ? await prisma.test_Report.findFirst({ where: { LogID } })
          : await prisma.report.findFirst({ where: { LogID } });
      }
      ...
      // Both checks above deliberately match hidden rows as well. If an admin
      // hid a report, re-submitting the identical report has to keep 409-ing
      // rather than quietly creating a fresh, visible copy of it.
      if (existing) {
