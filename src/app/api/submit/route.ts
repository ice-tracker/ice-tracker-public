...
const sensitiveLocations = new Set(["home", "workplace"]);
...
export async function getPopulationStatsCircle(
  centerLat: number,
  centerLon: number,
  radiusMiles: number
): Promise<any> {
...
async function processPoint(
  ...
  // If none of radii fit, default to 4 miles
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (type === "report") {
      ...
      let TotalPopulation;
      let Radius;
      if (sensitiveLocations.has(body.Location)) {
      ...
      const created = await prisma.report.create({
        data: {
          ...
          Latitude,
          Longitude,
          Address: sensitiveLocations.has(body.Location)
